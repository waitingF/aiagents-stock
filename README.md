...
**新增独立板块：多源宏观数据 × A股行业映射 × 优质标的筛选**

本次新增“🌏 宏观分析”板块，通过本地缓存、Tushare Pro、AKShare 和国家统计局发布稿抓取核心宏观数据，再调用 `.env` 中配置的 OpenAI 兼容大模型，对当前国内宏观经济形势进行研判，并映射到 A 股行业板块和代表性标的。

**核心功能：**
- 📊 **核心宏观数据多源兜底** — GDP 当季同比、GDP 环比、规上工业增加值同比、CPI、PPI、制造业 PMI、非制造业商务活动指数、综合 PMI
