...
from src.aiagents_stock.features.stock_analysis.report import display_pdf_export_section
from src.aiagents_stock.features.stock_analysis.repository import db
from src.aiagents_stock.features.realtime_monitor.manager import display_monitor_manager, get_monitor_summary
...
from src.aiagents_stock.features.selectors.main_force.ui import display_main_force_selector
from src.aiagents_stock.features.sector_strategy.ui import display_sector_strategy
...
from src.aiagents_stock.features.stock_analysis.service import (
    ...
    fetch_stock_data,
    parse_stock_list as parse_stock_list_service,
)
...
def main():
    ...
    if 'show_low_price_bull' in st.session_state and st.session_state.show_low_price_bull:
        from src.aiagents_stock.features.selectors.low_price_bull.ui import display_low_price_bull
        display_low_price_bull()
        return
    
    # 检查是否显示小市值策略
    if 'show_small_cap' in st.session_state and st.session_state.show_small_cap:
        from src.aiagents_stock.features.selectors.small_cap.ui import display_small_cap
        display_small_cap()
        return
    
    # 检查是否显示净利增长策略
    if 'show_profit_growth' in st.session_state and st.session_state.show_profit_growth:
        from src.aiagents_stock.features.selectors.profit_growth.ui import display_profit_growth
        display_profit_growth()
        return

    # 检查是否显示低估值策略
    if 'show_value_stock' in st.session_state and st.session_state.show_value_stock:
        from src.aiagents_stock.features.selectors.value_stock.ui import display_value_stock
        display_value_stock()
        return
    ...
    if 'show_portfolio' in st.session_state and st.session_state.show_portfolio:
        from src.aiagents_stock.features.portfolio.ui import display_portfolio_manager
    ...
    if 'show_macro_analysis' in st.session_state and st.session_state.show_macro_analysis:
        from src.aiagents_stock.features.macro_analysis.ui import display_macro_analysis
    ...
    if analyze_button and stock_input:
        ...
        if analysis_mode == "单个分析":
            ...
            # 批量股票分析
            # 解析股票代码列表
            stock_list = parse_stock_list_service(stock_input)
...
def run_batch_analysis(stock_list, period, batch_mode="顺序分析"):
    ...
    if batch_mode == "多线程并行":
        ...
        def analyze_with_progress(symbol):
            ...
            try:
                result = analyze_single_stock_for_batch_service(symbol, period, enabled_analysts_config, selected_model)
                with lock:
                    completed[0] += 1
