...
class NewsFlowEngine:
    ...
    def _init_modules(self):
        """初始化所有模块"""
        try:
            from src.aiagents_stock.features.news_flow.data import NewsFlowDataFetcher
            self.fetcher = NewsFlowDataFetcher()
        except Exception as e:
            logger.error(f"数据获取模块初始化失败: {e}")
        
        try:
            from src.aiagents_stock.features.news_flow.model import NewsFlowModel
            self.model = NewsFlowModel()
        except Exception as e:
            logger.error(f"流量模型模块初始化失败: {e}")
        
        try:
            from src.aiagents_stock.features.news_flow.sentiment import SentimentAnalyzer
            self.sentiment = SentimentAnalyzer()
        except Exception as e:
            logger.error(f"情绪分析模块初始化失败: {e}")
        
        try:
            from src.aiagents_stock.features.news_flow.agents import NewsFlowAgents
            self.agents = NewsFlowAgents()
        except Exception as e:
            logger.error(f"AI分析模块初始化失败: {e}")
        
        try:
            from src.aiagents_stock.features.news_flow.alert import NewsFlowAlertSystem
            self.alerts = NewsFlowAlertSystem()
        except Exception as e:
            logger.error(f"预警系统模块初始化失败: {e}")
        
        try:
            from src.aiagents_stock.features.news_flow.repository import news_flow_db
    ...
    def get_dashboard_data(self) -> Dict:
        ...
        try:
            ...
            # 调度器状态
            try:
                from src.aiagents_stock.features.news_flow.scheduler import news_flow_scheduler
