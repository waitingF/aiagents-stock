...
def display_dashboard():
    ...
    try:
        from src.aiagents_stock.features.news_flow.engine import news_flow_engine
    ...
    with col2:
        if st.button("📥 生成PDF报告", type="primary", key="gen_pdf_btn"):
            with st.spinner("正在生成PDF报告..."):
                try:
                    from src.aiagents_stock.features.news_flow.report import NewsFlowPDFGenerator
...
def display_alert_center():
    ...
    try:
        from src.aiagents_stock.features.news_flow.alert import news_flow_alert_system
        from src.aiagents_stock.features.news_flow.repository import news_flow_db
    except Exception as e:
        st.error(f"模块加载失败: {e}")
        return
